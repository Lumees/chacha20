...
Usage:
    python3 chacha20_soc.py --build
    python3 chacha20_soc.py --load
"""
...
from chacha20_litex import ChaCha20
...
class ChaCha20SoC(SoCCore):
    def __init__(self, sys_clk_freq: float = 100e6, **kwargs):
        ...
        SoCCore.__init__(self, platform,
            ...
            ident    = "ChaCha20 IP Test SoC - Arty A7-100T",
            **kwargs
        )
        ...
        # ── ChaCha20 IP ──────────────────────────────────────────────────
        self.submodules.chacha20 = ChaCha20(platform)
        self.add_csr("chacha20")
        ...
        self.add_csr("leds")
        self.comb += [
            leds[0].eq(self.chacha20.status.status[2]),  # busy
            leds[1].eq(self.chacha20.irq),                # done/irq
        ]


# ── Main ─────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="ChaCha20 SoC on Arty A7-100T")
    builder_args(parser)
    ...
    soc = ChaCha20SoC(**soc_core_argdict(args))
